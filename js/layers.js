...
function drawStaticBackground(levelSpec) {
    const buffer = document.createElement('canvas');
    buffer.width = 840 + 60;
    buffer.height = 660;
    ...
    function loadGrass() {
        loadImage(levelSpec.backimage).then(function(result) {
            GlassBackImage = result;
            loaded++;
        });

        loadImage(levelSpec.lowerimage).then(function(result) {
            GrassImage = result;
            loaded++;
        });

        loadImage(levelSpec.frontimage).then(function(result) {
